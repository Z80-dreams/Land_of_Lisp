...
(defparameter *board-size* 2) ; 3x3 board. This version can handle 3x3 grids easily in clisp, bigger boards needs more optimization. See ver 2.
...
(defun play-vs-computer (tree)
  (print-info tree)
  (cond ((null (caddr tree)) (announce-winner (cadr tree))) ;if no moves possible
...
(defun handle-human (tree)
  ...
  (let ((moves (caddr tree))) ; all trees of ((src-dst) player #board moves)
    (loop for move in moves
	  for n from 1
	  do (let ((action (car move))) ; take only (src-dst) cons-cell
	       (fresh-line)
	       (format t "~a. " n) ; print number n
	       (if action ; if cons-cell for action exists
		   (format t "~a -> ~a" (car action) (cadr action))
		   (princ "end turn")))) ; if con-cells doesn't exist.
    (fresh-line)
    (cadr (nth (1- (read)) moves)))) ; return new tree, as a branch of old tree.
...
(defun rate-position (tree player)
  (let ((moves (caddr tree))) ;all possible moves (sub-trees)
	...
	(let ((w (winners (cadr tree)))) ;if no moves are possible, pass #board to winners function to get winner
	  (if (member player w)
	      (/ 1 (length w)) ;get score for shared victory
	      0))))) ;0 score if leading to lose.

;;; Memoization of rate-position. If it sees the same tree and player combo, it
;;; doesn't need to go through the calculation again.
(let ((old-rate-position (symbol-function 'rate-position))
      (previous (make-hash-table)))
  (defun rate-position (tree player)
    (let ((tab (gethash player previous))) ; one hash table "tab" per player
      (unless tab
	(setf tab (setf (gethash player previous) (make-hash-table)))) ;make tab is doesn't exist.
      (or (gethash tree tab)
	  (setf (gethash tree tab)
		(funcall old-rate-position tree player))))))

;;; CLEAN FUNCTIONAL
;;; This function calls the rate-position recursively for each sub-tree.
(defun get-ratings (tree player)
  (mapcar (lambda (move)
	    (rate-position (cadr move) player))  ; cadr of moves is subtree.
	  (caddr tree))) ;caddr of tree is moves for that subtree

;;; DIRTY IMPERATIVE
;;; This function picks the optimal score for the ai player.
;;; This is imperative because it changes tree
(defun handle-computer (tree)
  (let ((ratings (get-ratings tree (car tree)))) ; get ratings for next player
    (cadr (nth (position (apply #'max ratings) ratings) (caddr tree))))) ;get max
...
(defun add-passing-move (board player spare-dice first-move moves)
  (if first-move
      moves
      (cons (list nil ; name of move, since it passing we name it nil
...
(defun attacking-moves (board cur-player spare-dice)
  (labels ((player (pos)
    ...
    (mapcan (lambda (src) ; like mapcar, except result is return as flattened list
	      (when (eq (player src) cur-player) ; make sure you only move your own dice
		(mapcan (lambda (dst) ; list all legal moves from this pos
			  (when (and (not (eq (player dst) cur-player)) ; don't move to a space you already own
				     (> (dice src) (dice dst))) ; only move if your pile is higher than opponents pile.
			    (list
			     (list (list src dst) ; move-name from-to
				   (game-tree (board-attack board cur-player src dst (dice src)) ; list subsecurent possible game trees.
					      cur-player
					      (+ spare-dice (dice dst)) ; add opponents dice to your own
					      nil))))) ; first move is nil
			(neighbors src)))) ; input list for the second mapcan
